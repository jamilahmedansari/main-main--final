...
export async function register() {
  ...
  if (process.env.NEXT_RUNTIME === "nodejs") {
    ...
    const { getShutdownManager } = await import(
      "./lib/server/graceful-shutdown"
    );
    ...
    shutdownManager.register("supabase", async () => {
      console.log("[Shutdown] Closing Supabase connections...");
