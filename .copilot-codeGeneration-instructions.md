...
- ✅ AI calls: Via Supabase Edge Functions (OpenAI only)
- ✅ Audit logging: `supabase.rpc('log_letter_audit', { ... })`
- ❌ Never: Direct OpenAI calls from Next.js
- ❌ Never: New Gemini code (migration in progress)
...
- `SERVICE_ROLE_KEY`
- `OPENAI_API_KEY` (set in Supabase dashboard for Edge Functions)
- `GEMINI_API_KEY` (legacy, to be removed after migration)
- `STRIPE_SECRET_KEY`
...
### Migration Strategy (Gemini → OpenAI)

**Current state:**

- Two files use Gemini: `app/api/generate-letter/route.ts` and `app/api/letters/[id]/improve/route.ts`
- These need migration to Supabase Edge Functions with OpenAI

**Migration approach:**

1. Create new Edge Functions in `supabase/functions/`
2. Update the two API routes to call Edge Functions instead of Gemini
3. Test thoroughly
4. Remove `GEMINI_API_KEY` from environment variables
5. Do NOT add any new Gemini code during this process
...
4. **All AI through Edge Functions** – No direct OpenAI/Gemini calls from Next.js
...
9. **Test locally first** – Use `supabase start` for local development
